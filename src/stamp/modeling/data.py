...
def tile_bag_dataloader(
    *,
    ...
    # Legacy single-pass heatmap weighting (per-slide H5 layout).
    heatmap_dir: Path | None = None,
    heatmap_score_key: str = "pos",
    heatmap_normalize: bool = True,
    heatmap_diagnostics_dir: Path | None = None,
    # Fold-aware prompt masks (consolidated HDF5 with /fold_<i>/ groups).
...
class BagDataset(Dataset[tuple[_Bag, _Coordinates, BagSize, _EncodedTarget]]):
    ...
    # Legacy heatmap-weighted features (single-pass, flat directory).
    heatmap_dir: Path | None = None
    heatmap_score_key: str = "pos"
    heatmap_normalize: bool = True
    heatmap_diagnostics_dir: Path | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.prompt_masks_path is not None and self.heatmap_dir is not None:
            # The two weighting mechanisms are alternatives: prompt_masks_path
            # is the fold-aware successor to the flat heatmap_dir layout.
            # Accepting both would double-weight features silently.
            raise ValueError(
                "prompt_masks_path and heatmap_dir are mutually exclusive; "
                "set exactly one"
            )
